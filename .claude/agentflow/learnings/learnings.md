...
## 2026-02-05: Dual-path migration creates divergence risk

When migrating files from `.orchestrator/` to `.claude/agentflow/`, both copies must be kept in sync until the old path is fully deprecated. The v1.0.2 fixes to `.orchestrator/scripts/` (shared file-lock.sh, create-pool.sh input validation) were not propagated to `.claude/agentflow/scripts/`, creating a situation where the path-resolver could silently select the unpatched version.

**Lesson**: Any fix applied to files under `.orchestrator/` must also be applied to their `.claude/agentflow/` counterparts, or better yet, use symlinks/single source of truth.
